...
const useFormStore = create<FormState>((set, get) => ({
  formDefinition: {
    ...
    options: {},
    slides: [{ elements: [{ id: randomId(), type: 'h1', text: 'My New Form' }] }],
  },
  selectedElementId: null,
  selectedSlideIndex: null,

  // --- ACTIONS ---
  setSelectedElementId: (id) => set({ selectedElementId: id, selectedSlideIndex: null }),
  setSelectedSlideIndex: (index) => set({ selectedSlideIndex: index, selectedElementId: null }),
  
  updateFormSettings: (newSettings) => set(produce((state: FormState) => {
    state.formDefinition.settings = { ...state.formDefinition.settings, ...newSettings };
  })),
  
  updateSlideSettings: (slideIndex, newOptions) => set(produce((state: FormState) => {
    const location = findSlideById(state.formDefinition, slideIndex);
    if (location) {
      location.slide.slideOptions = { ...(location.slide.slideOptions || {}), ...newOptions };
    }
  })),

  toggleStartSlide: (enabled) => set(produce((state: FormState) => {
    if (enabled) {
      if (!state.formDefinition.startSlide) {
        state.formDefinition.startSlide = { elements: [{ id: self.crypto.randomUUID(), type: 'h1', text: 'Welcome!' }] };
      }
    } else {
      delete state.formDefinition.startSlide;
      if (state.selectedSlideIndex === 'start') state.selectedSlideIndex = null;
    }
  })),

  toggleEndSlide: (enabled) => set(produce((state: FormState) => {
    if (enabled) {
      if (!state.formDefinition.endSlide) {
        state.formDefinition.endSlide = { elements: [{ id: self.crypto.randomUUID(), type: 'h1', text: 'Thank You!' }] };
      }
    } else {
      delete state.formDefinition.endSlide;
      if (state.selectedSlideIndex === 'end') state.selectedSlideIndex = null;
    }
  })),

  setFormDefinition: (definition) => set({ formDefinition: definition, selectedElementId: null, selectedSlideIndex: null }),

  saveToLocalStorage: () => {
    const currentState = get();
    localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(currentState.formDefinition));
  },

  updateElement: (elementId, newProps) => set(produce((state: FormState) => {
  ...
  deleteElement: (elementId) => set(produce((state: FormState) => {
  ...
  moveElement: (activeElementId, overElementId, overSlideId) => set(produce((state: FormState) => {
    const sourceLocation = findElementById(state.formDefinition, activeElementId);
    if (!sourceLocation) return;
    
    const [movedElement] = sourceLocation.slide.elements.splice(sourceLocation.elementIndex, 1);
    if (!movedElement) return;
    
    const destSlideLocation = findSlideById(state.formDefinition, overSlideId);
    if (!destSlideLocation) return;

    let destIndex: number;
    if (overElementId) {
      const overLocation = findElementById(state.formDefinition, overElementId);
      destIndex = overLocation ? overLocation.elementIndex : destSlideLocation.slide.elements.length;
    } else {
  ...
  addElementFromToolbox: (slideId, destinationIndex, element) => set(produce((state: FormState) => {
  ...
  addSlide: () => set(produce((state: FormState) => {
    state.formDefinition.slides.push({ elements: [] });
  })),

  deleteSlide: (slideIndex) => set(produce((state: FormState) => {
