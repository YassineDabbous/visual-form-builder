  ...
  return (
    <div className="space-y-4">
      ...
      <div className="p-3 border rounded-lg bg-gray-50/50 space-y-4">
        <h3 className="text-xs font-semibold text-gray-600 uppercase tracking-wider">{t('picture_options')}</h3>
        <BooleanToggle label={t('allow_multiple_selections')} checked={element.multiple || false} onChange={(e) => updateElement(element.id, { multiple: e.target.checked })} />
        <BooleanToggle label={t('supersize_pictures')} checked={element.supersize || false} onChange={(e) => updateElement(element.id, { supersize: e.target.checked })} />
        <BooleanToggle label={t('hide_text_labels')} checked={element.hideLabels || false} onChange={(e) => updateElement(element.id, { hideLabels: e.target.checked })} />
        <BooleanToggle label={t('hide_form_text')} description={t('hide_form_text_description')} checked={element.hideFormText || false} onChange={(e) => updateElement(element.id, { hideFormText: e.target.checked })} />
      </div>

      {element.multiple && (
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">{t('default_selections')}</label>
            <div className="space-y-2 p-3 border rounded-md bg-white">
              {choices.map((choice, index) => (
                <BooleanToggle key={index} label={choice.label || t('option_n', { index: index + 1 })} checked={checkedValues.includes(choice.value)} onChange={(e) => handleDefaultCheckedChange(choice.value, e.target.checked)} />
              ))}
              {choices.length === 0 && <p className="text-xs text-gray-400">{t('add_choices_to_set_defaults')}</p>}
            </div>
          </div>
        )}

      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">{t('choices')}</label>
        <div className="space-y-3">
          {choices.map((choice, index) => (
            <div key={index} className="p-3 border rounded-lg space-y-2 bg-white relative">
              ...
              <TextInput label={t('label')} value={choice.label} onChange={(e) => handlePropertyChange(index, 'label', e.target.value)} />
              <TextInput label={t('value')} value={choice.value} onChange={(e) => handlePropertyChange(index, 'value', e.target.value)} />
              <TextInput label={t('image_url')} value={choice.image} onChange={(e) => handlePropertyChange(index, 'image', e.target.value)} />
            </div>
          ))}
